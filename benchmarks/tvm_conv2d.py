...
def conv2d(B, C1, H, W, C2, Kx, Ky, dilx, dily, strx, stry, dtype):
    ...
    out = te.compute(
        ...
        lambda ib, ic, ip, iq: te.sum(
            I[ib, jc, ip * strx + jx * dilx, iq * stry + jy * dily] * F[ic, jc, jy, jx], axis=[jc, jx, jy]
        ),
    )
...
print("Execution time of this operator: %.3f ms" % (np.median(evaluator(i_tvm, w_tvm, out_tvm).results) * 1000))
...
print(
    triton.testing.do_bench(
        lambda: torch.nn.functional.conv2d(I_tch, W_tch, stride=(strx, stry), dilation=(dilx, dily))
    )
)
