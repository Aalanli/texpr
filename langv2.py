...
class Var:
    ...
    def __getitem__(self, indices: Union[NumExpr, Tuple[NumExpr,...]]):
        ...
        idx = [wrap_constexpr(v).value for v in indices] if isinstance(indices, tuple) else [wrap_constexpr(indices).value]
...
def matmul(a: Var, b: Var):
    ...
    c = grid_compute((m, n),
                     f=lambda i, j: 
                        grid_reduce(ir.ReduceOperations.Sum,
                                    (k,),
                                    f=lambda k: a[i, k] * b[k, j]))
    return c

def softmax(a: Var):
    assert len(tensor_shape(a)) == 1
    n = shape_of(a, 0)
    maxes = grid_reduce(ir.ReduceOperations.Max,
                        (n,),
                        lambda i: a[i])
    subs = grid_compute((n,),
                        lambda i: exp(a[i] - maxes))
    sums = grid_reduce(ir.ReduceOperations.Sum,
                       (n,),
                       lambda i: subs[i])
    y = grid_compute((n,),
                     lambda i: subs[i] / sums)
    return y
